...
export function LoginScreen() {
  const { googleEmail, authLoading, go, set, notify } = useDashboard()
  ...
  if (authLoading) {
    return (
      <div className="animate-[pop_.35s_ease] px-6 pt-10 pb-6 min-h-[700px] flex flex-col items-center justify-center">
        <div className="w-[60px] h-[60px] rounded-[18px] flex items-center justify-center text-white font-extrabold text-2xl mb-4 animate-pulse" style={{ background: 'linear-gradient(135deg,#2a6fdb,#5a93ef)' }}>S</div>
  ...
  return (
    <div className="animate-[pop_.35s_ease] px-6 pt-10 pb-6 min-h-[700px] flex flex-col">
      ...
      {!googleEmail && (
        <>
          <button onClick={signInWithGoogle} className="w-full border border-[#dfe3ea] bg-white rounded-[14px] p-3.5 mt-[30px] flex items-center justify-center gap-[11px] cursor-pointer shadow-[0_1px_2px_rgba(20,30,60,.06)]">
          ...
          <div className="flex items-center gap-3 mt-5">
            ...
            <span className="text-[11.5px] text-td-subtle font-semibold">or try demo mode</span>
            <div className="flex-1 h-px bg-[#e6eaf2]" />
          </div>
        </>
      )}
